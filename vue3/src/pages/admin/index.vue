  ...
  <view class="admin-page">
    <view class="header-card">
      ...
      <text class="header-desc">
        当前轮把管理员页从命令输入页升级为系统管理面板，覆盖用户管理、系统开关和设备/网关接入状态。
      </text>
    </view>
    ...
    <view v-if="activeSection === 'gateway'" class="panel-card">
      <text class="panel-title">设备 / 网关接入状态</text>
      ...
      <button class="primary-button" :loading="loadingStates.openclawStatus" @tap="refreshOpenClawStatus">
        刷新接入状态
      </button>
    </view>
  </view>
...
async function refreshOpenClawStatus() {
  try {
    await adminStore.loadOpenClawStatus()
    uni.showToast({
      title: '接入状态已刷新',
      icon: 'success',
    })
