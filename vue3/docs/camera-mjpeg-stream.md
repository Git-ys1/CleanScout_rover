...
- `CAMERA_TARGET_FPS=20` 表示 UbuntuPC 最多上送 20fps，不会重新编码，只转发 ESP32-CAM 原始 JPEG 帧。
- `CAMERA_STREAM_INTERVAL_MS=50` 表示 backend 最多按 20fps 向 H5 输出最新帧；不要再用 1000ms 作为输出循环，否则前端会退化成约 1fps。
