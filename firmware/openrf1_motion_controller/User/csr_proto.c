...
/*
 * USART2 接收中断只写环形缓冲区，命令解析放在主循环中完成。
 * 这样可以避免在中断里执行 strtok/strtod 等耗时操作，保护 50Hz 控制节拍。
 */
...
static int csr_proto_parse_line(char *line, csr_proto_command_t *command)
{
    ...
    /* W,a,b,c,d：四轮目标速度，单位 m/s，顺序固定为 CN1~CN4。 */
    if (strcmp(token, "W") == 0)
    {
...
void csr_proto_init(uint32_t baudrate)
{
    ...
    /* USART2 固定使用 PA2(TX)/PA3(RX)，115200 8N1。 */
...
void csr_proto_send_navdbg(const float *cmd, const float *ramp, const float *rt, const int16_t *out)
{
    ...
    {
        csr_proto_append_text(line, sizeof(line), ",");
        csr_proto_append_float3(line, sizeof(line), cmd[index]);
    }
    for (index = 0; index < CSR_CHANNEL_COUNT; index++)
    {
        csr_proto_append_text(line, sizeof(line), ",");
        csr_proto_append_float3(line, sizeof(line), ramp[index]);
    }
    for (index = 0; index < CSR_CHANNEL_COUNT; index++)
    {
        csr_proto_append_text(line, sizeof(line), ",");
