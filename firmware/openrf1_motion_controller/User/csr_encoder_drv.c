...
static TIM_TypeDef *csr_encoder_timer(csr_channel_t channel)
{
    /*
     * OpenRF1 原生编码器定时器映射：
     * CN1->TIM5，CN2->TIM3，CN3->TIM2 全重映射，CN4->TIM4。
     */
    switch (channel)
    {
...
static void csr_encoder_init_cn3(void)
{
    ...
    /*
     * TIM2 全重映射后 CH1/CH2 位于 PA15/PB3。
     * main() 已提前关闭 JTAG、保留 SWD，保证这两个引脚可用。
     */
...
int32_t csr_encoder_read_and_reset(csr_channel_t channel)
{
    ...
    /*
     * 20ms 窗口内读取有符号 16 位增量并清零计数器。定时器方向由硬件
     * 正交解码决定，再通过 encoder_dir_sign 统一为车体速度语义。
     */
