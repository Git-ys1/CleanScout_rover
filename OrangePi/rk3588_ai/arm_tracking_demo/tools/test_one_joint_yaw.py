#!/usr/bin/env python3
"""Small yaw-axis test. Defaults to dry-run; real output requires explicit flags."""
...
def main():
    ...
    parser.add_argument("--yaw", type=float, default=0.10)
    ...
    driver.connect()
    driver.set_yaw(args.yaw, args.duration_ms)
    time.sleep(max(args.hold_s, args.duration_ms / 1000.0 + 0.1))
    driver.stop([int(driver.config["yaw_servo_index"])])
    driver.close()
