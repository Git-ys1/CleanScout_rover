...
from arm_grasp_pipeline.geometry import HandEye
...
def main() -> int:
    ...
    current_joints = config.get("current_reference_joints_rad")
    ...
    if backend == "official_f103":
        ...
        reference_pwms = parse_pwms(args.current_pwms) if args.current_pwms else list(
            grasp_cfg.get("retry_pose_pwms",
                          kinematics_cfg.get("current_reference_pwms", [1380, 1909, 1900, 620, 1500, 1500])))
        reference_matrix = kin.forward_matrix_from_pwm(reference_pwms)
        arm = ArmMotion(adapter, kinematics=kin, reference_ee_matrix=reference_matrix)
    elif backend == "ros2_learning":
        arm = ArmMotion(adapter, current_joints_rad=current_joints)
    else:
    ...
    current_pwms = list(reference_pwms) if backend == "official_f103" else None
    ...
    hand_names = {"x", "y", "z", "roll_deg", "pitch_deg", "yaw_deg"}
            ...
            if (frame_count - 1) % (args.skip + 1) == 0:
                ...
                if selected is not None and args.auto_center and current_pwms is not None:
                    ...
                    if updates and now - last_center_command_time >= center_interval_s:
                        ...
                        arm.set_reference_ee_matrix(kin.forward_matrix_from_pwm(current_pwms))
                ...
                if target is not None:
                    ...
                    lock_record = {
                        ...
                        "point_arm_camera_axes_m": None if lock_debug is None else list(lock_debug.point_arm_camera_axes_m),
                    }
    ...
    print("SUMMARY frames={} inferences={} locked={} center_commands={} dry_run={} hand_eye_calibrated={}".format(
