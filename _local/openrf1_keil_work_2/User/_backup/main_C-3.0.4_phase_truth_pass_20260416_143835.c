...
#define CSR_PWM_PER_MPS            4000.0f
#define CSR_PWM_MIN                160
#define CSR_PWM_MAX                500
#define CSR_PWM_TOP                2000
#define CSR_PWM_LOW                0
#define CSR_PWM_HIGH               CSR_PWM_TOP
#define CSR_BOOT_PHASE_TEST        1
...
static const csr_wheel_config_t g_wheels[CSR_WHEEL_COUNT] =
{
	 ...
	 * motor_sign maps protocol-positive to the observed forward direction.
	 */
...
static void csr_cn_set_truth(uint8_t index, int16_t signed_pwm)
{
	...
	/* Bottom truth mode: sign selects forward/reverse only.
	 * PWM magnitude is intentionally ignored in this bring-up path.
	 */
	csr_cn_set_direct_truth(index, (signed_pwm > 0) ? 1 : 0);
}
