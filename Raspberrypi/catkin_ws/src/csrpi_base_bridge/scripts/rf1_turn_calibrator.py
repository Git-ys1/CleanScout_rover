...
class Rf1TurnCalibrator:
    ...
    def wait_for_sensors(self):
        ...
        while not rospy.is_shutdown():
            ...
            if time.monotonic() >= deadline:
                raise RuntimeError(
                    "sensor timeout: wheels=%s lidar_pose=%s command_subscribers=%d"
                    % (
                        self.latest_wheels is not None,
    ...
    def run(self):
        rospy.loginfo("Waiting for RF1, lidar matcher, and command gate")
        self.wait_for_sensors()
        rospy.loginfo(
            "Starting %s turn using encoder and lidar evidence",
...
def main():
    ...
    try:
        ...
        if result["k_from_lidar_m"] is None:
